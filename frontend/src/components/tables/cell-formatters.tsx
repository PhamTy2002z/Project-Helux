...
export function linkifyCell({
  ...
  if (block) {
    return (
      <Link href={href} title={title} className={cn("group block", className)}>
        <p
          className={cn(
            "text-sm font-medium text-strong group-hover:text-blue-600",
            labelClassName,
          )}
        >
          {label}
  ...
  return (
    <Link
      ...
      title={title}
      className={cn(
        "text-sm font-medium text-[color:var(--text)] hover:text-blue-600",
        className,
      )}
    >
      {label}
...
export function dateCell(
  ...
  return (
    <span className={cn("text-sm text-[color:var(--text)]", className)}>
      {display ?? fallback}
    </span>
  );
}
