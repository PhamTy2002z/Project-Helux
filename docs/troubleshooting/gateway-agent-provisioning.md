...
VisgniteAI now uses a fast convergence policy for wake/check-in:
...
1. VisgniteAI provisions/updates the agent and sends wake.
...
3. Verify agent can reach VisgniteAI API and send heartbeat with `X-Agent-Token`.
...
3. Trigger agent update/provision from VisgniteAI.
4. Watch logs for:
   - `lifecycle.queue.enqueued`
