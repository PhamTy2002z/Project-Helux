...
VisgniteAI supports both secure (`wss://`) and non-secure (`ws://`) WebSocket connections to gateways.
...
When enabled, VisgniteAI skips TLS certificate verification for that gateway connection.
